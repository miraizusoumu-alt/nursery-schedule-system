...
function createWorkLimitProfiles(profiles, targetMonth, currentDays) {
  ...
  return normalizeAutomaticWorkLimitProfiles(profiles.map((profile) => ({
    ...
    existingDays: currentDays.filter((day) => day.staffId === profile.id).map((day) => ({
      date: day.date,
...
export function evaluateCurrentDraftSchedule({
  ...
  const workLimits = createWorkLimitProfiles(profiles, targetMonth, days);
