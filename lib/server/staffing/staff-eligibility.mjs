...
import { nationalHolidayForDate } from "./national-holidays.mjs";
...
function evaluateStaffForQuarterHourSlot(staff, slot, options = {}) {
  ...
  const nationalHoliday = nationalHolidayForDate(staff.nationalHolidays, slot.date);
  const holidayWorkAllowed = workCondition?.holidayWorkAllowed !== false;
  const violatesHolidayWorkAvailability = Boolean(nationalHoliday) && !holidayWorkAllowed;
  ...
  else if (violatesHolidayWorkAvailability) exclusionReasons.push("HOLIDAY_NOT_AVAILABLE");
  ...
  const eligible = isActiveOnDate
    ...
    && !violatesHolidayWorkAvailability
  ...
  return {
    ...
    assignedRoles,
    validQualifications,
    isNationalHoliday: Boolean(nationalHoliday),
    nationalHoliday,
    holidayWorkAllowed,
    violatesHolidayWorkAvailability,
    isEmployedOnDate,
