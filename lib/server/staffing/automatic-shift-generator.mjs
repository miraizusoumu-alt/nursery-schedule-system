...
function removableFullTimeAssignments(slots, slotIndex, incomingEvaluation, staffById) {
  const slot = slots[slotIndex];
  const removableAssignmentSources = new Set([
    "automatic",
    "automatic_childcare",
    "phase1_shortage_refill",
    "capacity_transfer_shortage_refill",
  ]);
  return (slot.assignedStaff ?? []).filter((assigned) => {
    ...
    if (assigned.assignmentSource && !removableAssignmentSources.has(assigned.assignmentSource)) return false;
...
function contiguousMinimumBlockWindows(
  slots,
  date,
  minimumMinutes,
  { shortageAnchorRequired = true } = {},
) {
  ...
  return windows.filter((indexes) => {
    return !shortageAnchorRequired
      || indexes.some((index) => slots[index].childcareWorkerShortage > 0);
  }).sort((left, right) => {
...
function simulatePartTimeMinimumBlock({
  ...
  if (shortageAnchorRequired
    && !windowIndexes.some((index) => currentSlots[index].childcareWorkerShortage > 0)) return null;
  ...
  for (const slotIndex of windowIndexes) {
    ...
    if (assignedStaff.length >= slot.requiredChildcareWorkers || replacementStaffId) {
      const replacementCandidates = removableFullTimeAssignments(
        trialSlots,
        slotIndex,
        evaluation,
        staffById,
      );
      const replacement = replacementStaffId
        ? replacementCandidates.find((entry) => entry.staffId === replacementStaffId)
  ...
  if (projectedMinutes < minimumMinutes
    || (shortageAnchorRequired && shortageAnchorReductions === 0)) return null;
...
function staffDateAssignmentIndexes(slots, staffId, date) {
  return slots.flatMap((slot, index) => (
    slot.date === date && (slot.assignedStaff ?? []).some((entry) => entry.staffId === staffId)
      ? [index]
      : []
  ));
}

function hasContiguousStaffDateAssignment(slots, staffId, date) {
  const indexes = staffDateAssignmentIndexes(slots, staffId, date);
  return indexes.every((slotIndex, index) => {
    if (index === 0) return true;
    return slots[indexes[index - 1]].endTime === slots[slotIndex].startTime;
  });
}

function shortagesDoNotIncrease(before, after) {
  return before.every((slot, index) => (
    after[index].childcareWorkerShortage <= slot.childcareWorkerShortage
    && after[index].licensedNurseryTeacherShortage <= slot.licensedNurseryTeacherShortage
  ));
}

function capacityTransferSources({
  slots,
  targetSlotIndex,
  staff,
  workloadProfiles,
  ...
  const previous = slots[targetSlotIndex - 1];
  const continuingStaffIds = previous?.date === targetSlot.date && previous.endTime === targetSlot.startTime
    ? new Set(previous.assignedStaff.map((entry) => entry.staffId))
    : new Set();
  const assignedIds = new Set(targetSlot.assignedStaff.map((entry) => entry.staffId));
  return staff.flatMap((profile) => {
    if (assignedIds.has(profile.id)) return [];
    if (activeWorkConditionForDate(profile, targetSlot.date)?.employmentType !== "常勤") return [];
    if (staffDateAssignmentIndexes(slots, profile.id, targetSlot.date).length === 0) return [];
    const eligibility = evaluateStaffAutomaticPlacementEligibilityForQuarterHourSlot(
      withGeneratedWorkDays(profile, state.generatedDates),
      targetSlot,
      {
        selectedAvailabilityCandidateId: selectedAvailabilityByStaffDate.get(
          staffDateWorkKey(profile.id, targetSlot.date),
        )?.candidateId ?? null,
      },
    );
    if (!eligibility.automaticPlacementEligible) return [];
    const workLimit = evaluateAutomaticWorkLimitAssignment({
      profile: workLimitProfiles.get(profile.id),
      generatedMinutes: state.generatedWorkMinutesByStaffDate,
      date: targetSlot.date,
      breakUnavailableStaffDates,
    });
    if (workLimit.eligible) return [];
    return [{ ...eligibility, workLimit }];
  }).sort((left, right) => compareBySelectionPriority(left, right, {
    continuingStaffIds,
    workloadProfiles,
    generatedMinutes: state.generatedMinutes,
  }));
}

function simulateCapacityTransfer({
  currentSlots,
  targetSlotIndex,
  sourceEvaluation,
  incomingProfile,
  ...
  const incomingWorkLimitProfile = workLimitProfiles.get(incomingProfile.id);
  const incomingCondition = activePartTimeWorkCondition(
    incomingWorkLimitProfile?.workConditions,
    targetSlot.date,
  );
  if (!incomingCondition) return null;
  const preference = workTimePreferenceForDate(
    incomingWorkLimitProfile.schedulePreferences,
    targetSlot.date,
  );
  const minimumMinutes = partTimeDailyMinimumMinutes(incomingCondition, preference);
  if (!minimumMinutes) return null;
  const initialState = generatedAssignmentState(currentSlots, workloadProfiles);
  const initialIncomingMinutes = projectedDailyAutomaticWorkMinutes(
    incomingWorkLimitProfile,
    initialState.generatedWorkMinutesByStaffDate,
    targetSlot.date,
    { breakUnavailableStaffDates },
  );
  if (initialIncomingMinutes !== 0) return null;
  if (!windowIndexes.every((index) => currentSlots[index].assignedStaff.some((entry) => (
    entry.staffId === sourceEvaluation.staffId
  )))) return null;

  const block = simulatePartTimeMinimumBlock({
    currentSlots,
    ...
    breakUnavailableStaffDates,
    selectedAvailabilityByStaffDate,
    shortageAnchorRequired: false,
    replacementStaffId: sourceEvaluation.staffId,
  });
  if (!block || block.replacements.length !== windowIndexes.length) return null;
  if (assignmentDisruption(block.slots, targetSlotIndex, sourceEvaluation.staffId) === 0) return null;

  const state = generatedAssignmentState(block.slots, workloadProfiles);
  const sourceProfile = staffById.get(sourceEvaluation.staffId);
  const targetEvaluation = evaluateAutomaticCandidate({
    profile: sourceProfile,
    slot: block.slots[targetSlotIndex],
    ...state,
    workLimitProfiles,
    breakUnavailableStaffDates,
    selectedAvailabilityByStaffDate,
  });
  if (!targetEvaluation.automaticPlacementEligible) return null;
  const trialSlots = [...block.slots];
  trialSlots[targetSlotIndex] = recalculateAssignedCounts(
    block.slots[targetSlotIndex],
    [...block.slots[targetSlotIndex].assignedStaff, {
      ...targetEvaluation,
      assignmentSource: "capacity_transfer_shortage_refill",
      continuedFromPreviousSlot: true,
      workloadBeforeSlot: workloadSnapshot(
        sourceEvaluation.staffId,
        workloadProfiles,
  ...
  if (!hasContiguousStaffDateAssignment(
    trialSlots,
    sourceEvaluation.staffId,
    targetSlot.date,
  )) return null;
  if (!shortagesDoNotIncrease(currentSlots, trialSlots)) return null;
  if (trialSlots[targetSlotIndex].childcareWorkerShortage >= targetSlot.childcareWorkerShortage) return null;

  const finalState = generatedAssignmentState(trialSlots, workloadProfiles);
  return {
    slots: trialSlots,
    date: targetSlot.date,
    targetStartTime: targetSlot.startTime,
    targetEndTime: targetSlot.endTime,
    sourceStaffId: sourceEvaluation.staffId,
    incomingStaffId: incomingProfile.id,
    blockStartTime: trialSlots[windowIndexes[0]].startTime,
    blockEndTime: trialSlots[windowIndexes.at(-1)].endTime,
    transferredMinutes: windowIndexes.length * 15,
    sourceMinutesBefore: initialState.generatedWorkMinutesByStaffDate.get(
      staffDateWorkKey(sourceEvaluation.staffId, targetSlot.date),
    ) ?? 0,
    sourceMinutesAfter: finalState.generatedWorkMinutesByStaffDate.get(
      staffDateWorkKey(sourceEvaluation.staffId, targetSlot.date),
    ) ?? 0,
    incomingMinutesBefore: initialState.generatedWorkMinutesByStaffDate.get(
      staffDateWorkKey(incomingProfile.id, targetSlot.date),
    ) ?? 0,
    incomingMinutesAfter: finalState.generatedWorkMinutesByStaffDate.get(
      staffDateWorkKey(incomingProfile.id, targetSlot.date),
    ) ?? 0,
  };
}

export function enumerateAutomaticCapacityTransferCandidates({
  placement,
  staffProfiles = [],
  workloadFairnessProfiles = [],
}) {
  if (!placement || !Array.isArray(placement.slots) || !Array.isArray(staffProfiles)) {
    throw new TypeError("capacity transfer候補には配置結果と職員候補を指定してください。");
  }
  ...
  for (const profile of staff) {
    if (!profile?.id || staffIds.has(profile.id)) {
      throw new TypeError("職員IDは重複しない値を指定してください。");
    }
    staffIds.add(profile.id);
  }
  const slots = placement.slots;
  const workloadProfiles = normalizeWorkloadFairnessProfiles(workloadFairnessProfiles, staffIds);
  ...
  const selectedAvailabilityByStaffDate = new Map(
    (placement.selectedAvailabilityCandidates ?? []).map((selection) => [
      staffDateWorkKey(selection.staffId, selection.date),
      selection,
    ]),
  );
  const staffById = new Map(staff.map((entry) => [entry.id, entry]));
  const incomingProfiles = staff.filter((profile) => (
    workLimitProfiles.get(profile.id)?.workConditions.some((condition) => (
      condition.employmentType === "非常勤"
    ))
  )).sort((left, right) => staffSortKey(left).localeCompare(staffSortKey(right)));
  const candidates = [];

  for (let targetSlotIndex = 0; targetSlotIndex < slots.length; targetSlotIndex += 1) {
    if (slots[targetSlotIndex].childcareWorkerShortage <= 0) continue;
    const targetSlot = slots[targetSlotIndex];
    const sources = capacityTransferSources({
      slots,
      targetSlotIndex,
      staff,
      workloadProfiles,
    ...
    for (const sourceEvaluation of sources) {
      for (const incomingProfile of incomingProfiles) {
        if (incomingProfile.id === sourceEvaluation.staffId) continue;
        const condition = activePartTimeWorkCondition(
          workLimitProfiles.get(incomingProfile.id)?.workConditions,
          targetSlot.date,
        );
        if (!condition) continue;
        const preference = workTimePreferenceForDate(
          workLimitProfiles.get(incomingProfile.id).schedulePreferences,
          targetSlot.date,
        );
        const minimumMinutes = partTimeDailyMinimumMinutes(condition, preference);
        const windows = contiguousMinimumBlockWindows(
          slots,
          targetSlot.date,
          minimumMinutes,
          { shortageAnchorRequired: false },
        ).filter((indexes) => indexes.every((index) => (
          slots[index].assignedStaff.some((entry) => entry.staffId === sourceEvaluation.staffId)
        )));
        let selected = null;
        for (const windowIndexes of windows) {
          selected = simulateCapacityTransfer({
            currentSlots: slots,
            targetSlotIndex,
            sourceEvaluation,
            incomingProfile,
  ...
  return candidates.sort((left, right) => {
    return left.date.localeCompare(right.date)
      || left.targetStartTime.localeCompare(right.targetStartTime)
      || left.sourceStaffId.localeCompare(right.sourceStaffId)
      || left.incomingStaffId.localeCompare(right.incomingStaffId)
      || left.blockStartTime.localeCompare(right.blockStartTime);
  });
}
