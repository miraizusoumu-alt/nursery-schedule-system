...
function compareByContinuity(left, right, continuingStaffIds) {
...
function selectEvaluations(evaluations, requiredChildcareWorkers, requiredLicensedNurseryTeachers, continuingStaffIds) {
  ...
  const licensed = eligible
    ...
    .sort((left, right) => compareByContinuity(left, right, continuingStaffIds));
...
export function calculateAutomaticChildcareShift(requirementSlots, staffProfiles) {
  ...
  const results = slots.map((slot) => {
    ...
    const evaluations = staff.map((entry) => {
      return evaluateStaffAutomaticPlacementEligibilityForQuarterHourSlot(
