...
export function validateScheduleSegments(segments, options = {}) {
  ...
  const normalized = segments.map((segment) => {
    ...
    const startMinutes = timeToMinutes(segment.startTime, "開始時刻");
    const endMinutes = timeToMinutes(segment.endTime, "終了時刻");
    ...
    if (startMinutes < minimum || endMinutes > maximum) {
      throw new StaffScheduleValidationError("OUTSIDE_SCHEDULE_RANGE", "勤務区分は06:30から20:30の範囲で指定してください。");
    }
