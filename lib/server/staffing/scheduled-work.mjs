...
export function countsAsScheduledWorkDay(day, options = {}) {
  const validated = validateScheduleDay(day, options);
...
export function calculateConsecutiveWorkWarnings(days, options = {}) {
  ...
  for (const day of sorted) {
    ...
    if (countsAsScheduledWorkDay(day, options)) {
      streak += 1;
...
export function evaluateConsecutiveWorkLimitForDate(days, proposedDate, options = {}) {
  if (!Array.isArray(days) || !Array.isArray(options.priorDays ?? [])) {
    throw new StaffScheduleValidationError("INVALID_DAYS", "日別シフトを配列で指定してください。");
  }
  assertDateKey(proposedDate);
  const maxConsecutiveDays = options.maxConsecutiveDays ?? MAX_CONSECUTIVE_WORK_DAYS;
  if (!Number.isInteger(maxConsecutiveDays) || maxConsecutiveDays < 1) {
    throw new StaffScheduleValidationError("INVALID_CONSECUTIVE_RULE", "連続勤務日数のルールが正しくありません。");
  }
  ...
  const existing = byDate.get(proposedDate);
  byDate.set(proposedDate, {
    staffId: options.staffId ?? existing?.staffId,
    date: proposedDate,
    dayType: "work",
    segments: existing?.segments ?? [],
  });

  function adjacentWorkDate(startDate, offset) {
    const date = dateFromKey(startDate);
    date.setUTCDate(date.getUTCDate() + offset);
    const adjacent = dateKey(date);
    const day = byDate.get(adjacent);
    return day && countsAsScheduledWorkDay(day, options) ? adjacent : null;
  }

  let startDate = proposedDate;
  let endDate = proposedDate;
  let consecutiveDays = 1;
  while (true) {
    const previous = adjacentWorkDate(startDate, -1);
    if (!previous) break;
    startDate = previous;
    consecutiveDays += 1;
  }
  while (true) {
    const next = adjacentWorkDate(endDate, 1);
    if (!next) break;
    endDate = next;
    consecutiveDays += 1;
  }
  ...
  return {
    ...
    consecutiveDays,
    maxConsecutiveDays,
    violatesLimit: consecutiveDays > maxConsecutiveDays,
  };
}
