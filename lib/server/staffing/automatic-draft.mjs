...
export function buildAutomaticSchedulePreview({
  ...
  const childcareStaffing = mergeContiguousShortages(unresolved.staffingShortages, "childcareWorkerShortage");
  const licensedStaffing = mergeContiguousShortages(unresolved.staffingShortages, "licensedNurseryTeacherShortage");
  const issues = {
    childcareStaffing,
    licensedStaffing,
    daysOff,
    consecutiveWork,
    breaks: enrichStaffIssues(unresolved.breaks, profiles),
  };
  return {
    targetMonth: month,
