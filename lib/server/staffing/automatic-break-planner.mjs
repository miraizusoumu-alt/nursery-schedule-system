...
import { validateScheduleDay } from "./scheduled-work.mjs";
...
function eligibleReliefCandidates({
  ...
  return staffProfiles.flatMap((profile) => {
    ...
    if (!evaluation.automaticPlacementEligible || (requireLicensed && !evaluation.licensedEligible)) return [];
    return [evaluation];
  });
}
...
function simulateCoverage({
  ...
  for (const slot of window) {
    ...
    while (true) {
      ...
      const requireLicensed = shortage.licensedNurseryTeacherShortage > 0;
      if (!requireLicensed && shortage.childcareWorkerShortage === 0) break;
...
export function planAutomaticBreaks({
  ...
  for (const segment of existing.breaks) {
  ...
  for (const request of requests) {
    ...
    if (existingCoverages.length > 0) {
      ...
      outcomes.push({
        ...
        placementStatus: success ? "preserved_existing" : "unresolved",
        ...
        reliefRequired: existingCoverages.some(({ actions }) => actions.length > 0),
        reliefStaffIds: [...new Set(existingCoverages.flatMap(({ actions }) => {
