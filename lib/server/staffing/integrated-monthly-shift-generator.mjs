...
export function calculateIntegratedMonthlyAutomaticShift({
  ...
  const childcareSegments = mergeChildcareAssignmentsIntoSegments(placement.slots);
  const shortages = placement.slots.flatMap((slot) => {
  ...
  return {
    ...
    placement,
    childcareSegments,
    staffWorkloads: placement.staffWorkloads,
    shortages,
  };
}
