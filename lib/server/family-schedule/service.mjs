...
import { calculateRequiredChildcareStaff } from "../staffing/required-childcare-staff.mjs";
import { aggregateStaffingRequirementsToQuarterHours } from "../staffing/quarter-hour-requirements.mjs";
import {
  connectRequirementWithStaffCandidates,
  evaluateStaffAutomaticPlacementEligibilityForQuarterHourSlot,
} from "../staffing/staff-eligibility.mjs";
import {
  CURRENT_STAFF_CLASSIFICATION_CAPABILITIES,
  CURRENT_STAFF_CLASSIFICATION_LIMITATIONS,
  loadStaffCandidateProfiles,
} from "../staffing/staff-candidate-repository.mjs";
...
function fiscalAgeOnAprilFirst(birthDate, targetMonth) {
  ...
  const fiscalYear = targetMonthNumber >= 4 ? targetYear : targetYear - 1;
  const [birthYear, birthMonth, birthDay] = birthDate.split("-").map(Number);
  let age = fiscalYear - birthYear;
  if (birthMonth > 4 || (birthMonth === 4 && birthDay > 1)) age -= 1;
  return age;
}

function shortestUniqueFamilyPrefix(lastName, group) {
  if (!lastName) return "";
  for (let length = 1; length <= lastName.length; length += 1) {
    const prefix = lastName.slice(0, length);
    if (group.filter((entry) => entry.lastName.slice(0, length) === prefix).length === 1) return prefix;
  }
  return "";
}

function childDisplayNames(children) {
  const prepared = children.map((child) => {
    const identity = resolveChildIdentity(child);
    return {
      childId: child.child_id ?? child.id,
      baseName: identity.firstName || identity.name || "氏名未設定",
      lastName: identity.lastName,
    };
  });
  const byBaseName = new Map();
  for (const entry of prepared) {
    const group = byBaseName.get(entry.baseName) ?? [];
    group.push(entry);
    byBaseName.set(entry.baseName, group);
  }
  const result = new Map();
  for (const group of byBaseName.values()) {
    const sorted = [...group].sort((left, right) => String(left.childId).localeCompare(String(right.childId)));
    for (const [index, entry] of sorted.entries()) {
      if (group.length === 1) {
        result.set(entry.childId, entry.baseName);
        continue;
      }
      const prefix = shortestUniqueFamilyPrefix(entry.lastName, group);
      result.set(entry.childId, prefix ? `${entry.baseName}（${prefix}）` : `${entry.baseName}（${index + 1}）`);
    }
  }
  return result;
}

function countAtSlot(children, dateIndex, slot) {
  return children.reduce((summary, child) => {
    if (child.submissionStatus !== "submitted") return summary;
    const day = child.days[dateIndex];
    if (day?.usageStatus !== "using") return summary;
    const arrival = timeToMinutes(day.arrivalTime);
    const departure = timeToMinutes(day.departureTime);
    if (arrival === null || departure === null || arrival > slot || slot > departure) return summary;
    summary.total += 1;
    summary.byAgeGroup[child.ageGroup] += 1;
    summary.childNames.push(child.name);
    return summary;
  }, { total: 0, byAgeGroup: { "0歳児": 0, "1歳児": 0, "2歳児": 0 }, childNames: [] });
}

function staffingForCount(count) {
  return calculateRequiredChildcareStaff({
    zeroYearOldCount: count.byAgeGroup["0歳児"],
    oneYearOldCount: count.byAgeGroup["1歳児"],
    twoYearOldCount: count.byAgeGroup["2歳児"],
  });
}

function staffingDisplayChanged(previous, current) {
  return previous.zeroYearOldCount !== current.zeroYearOldCount
    || previous.oneYearOldCount !== current.oneYearOldCount
    || previous.twoYearOldCount !== current.twoYearOldCount
    || previous.totalChildren !== current.totalChildren
    || previous.requiredChildcareWorkers !== current.requiredChildcareWorkers
    || previous.requiredLicensedNurseryTeachers !== current.requiredLicensedNurseryTeachers;
}

function staffingRequirementRecord(date, time, staffing) {
  return {
    date,
    time,
    zeroYearOldCount: staffing.zeroYearOldCount,
    oneYearOldCount: staffing.oneYearOldCount,
    twoYearOldCount: staffing.twoYearOldCount,
    totalChildren: staffing.totalChildren,
    zeroYearOldContribution: staffing.zeroYearOldContribution,
    oneTwoYearOldContribution: staffing.oneTwoYearOldContribution,
    ageBasedRequirement: staffing.ageBasedRequirement,
    totalChildrenRuleRequirement: staffing.totalChildrenRuleRequirement,
    totalChildrenThreeToOneRequirement: staffing.totalChildrenThreeToOneRequirement,
    minimumStaffRequirement: staffing.minimumStaffRequirement,
    requiredChildcareWorkers: staffing.requiredChildcareWorkers,
    requiredLicensedNurseryTeachers: staffing.requiredLicensedNurseryTeachers,
    requiredMealStaff: staffing.requiredMealStaff,
    appliedRules: staffing.appliedRules,
    calculationBreakdown: staffing.calculationBreakdown,
  };
}
...
function tokyoCalendarParts(instant) {
  const shifted = new Date(instant.getTime() + TOKYO_OFFSET_MINUTES * 60_000);
  return {
...
function suggestedParentTargetMonth(instant) {
  const { year, month, day } = tokyoCalendarParts(instant);
  return shiftedMonth(year, month, day >= 15 ? 1 : 0);
}
...
function isFamilyCooperationDay(closure) {
  return Boolean(closure?.parent_input_allowed) || closure?.type === "family_cooperation";
}

function isHardClosure(dateKey, closure) {
  return weekdayNumber(dateKey) === 0 || Boolean(closure && !isFamilyCooperationDay(closure));
}
...
function assertSubmissionAccess(access) {
  if (!access.editable || !access.submittable) {
    if (access.submissionLocked) {
      throw new AuthError("SUBMISSION_LOCKED", "提出済みのため、ご自身では変更できません。変更が必要な場合は園へLINEまたは直接ご連絡ください。", 409);
    }
    throw new AuthError("SUBMISSION_LOCKED", "現在は編集できません。変更が必要な場合は園へご連絡ください。", 409);
  }
}
...
function validateTimePair(arrivalTime, departureTime) {
  ...
  if (arrivalMinutes < OPENING_MINUTES || departureMinutes > CLOSING_MINUTES) {
    throw new AuthError("OUTSIDE_OPENING_HOURS", "利用時間は7:00から20:00の範囲で入力してください。");
  }
}
...
function normalizeChildManagementInput(input, { existing = null } = {}) {
  ...
  const lastNameHiragana = requiredText(input?.lastNameKana, "INVALID_KANA", "姓（かな）を入力してください。");
  const firstNameHiragana = requiredText(input?.firstNameKana, "INVALID_KANA", "名（かな）を入力してください。");
  if (!/^[ぁ-ゖー]+$/.test(lastNameHiragana) || !/^[ぁ-ゖー]+$/.test(firstNameHiragana)) {
    throw new AuthError("INVALID_KANA", "姓（かな）・名（かな）は全角ひらがなで入力してください。");
  }
  const lastNameKana = hiraganaToKatakana(lastNameHiragana);
  const firstNameKana = hiraganaToKatakana(firstNameHiragana);
  ...
  const familyActiveFrom = familyId ? requiredDate(input?.familyActiveFrom, "保護者・家庭の登録開始日") : null;
  const familyActiveTo = familyId ? optionalDate(input?.familyActiveTo, "保護者・家庭の登録終了日") : null;
  if (familyId && familyActiveTo && familyActiveFrom > familyActiveTo) {
    throw new AuthError("INVALID_DATE_RANGE", "保護者・家庭の登録終了日は開始日以後にしてください。");
  }
  if (familyId && (familyActiveFrom < enrollmentDate || (withdrawalDate && (!familyActiveTo || familyActiveTo > withdrawalDate)))) {
    throw new AuthError("MEMBERSHIP_OUTSIDE_ENROLLMENT", "保護者・家庭の登録期間は園児の在籍期間内にしてください。");
  }
...
function makeInitialDay({ dateKey, child, closuresByDate, patternsByWeekday, timestamp }) {
  ...
  if (isHardClosure(dateKey, closure)) {
    return {
      id: randomUUID(),
      monthlyScheduleId: null,
      date: dateKey,
...
function statusLabel(submission, access) {
  if (access.resubmissionAllowed && submission?.status === "draft") return "再提出のため修正中";
  if (access.resubmissionAllowed) return "再提出できます";
  if (access.submissionLocked) return "提出済み";
  if (!access.editable) return "閲覧のみ";
...
export function createFamilyScheduleService({ database, now = () => new Date() }) {
  ...
  function parentSelectablePeriods() {
  ...
  function targetPeriodOrUnavailable(requestedPeriodId = "") {
    const periods = parentSelectablePeriods();
    const normalizedPeriodId = String(requestedPeriodId ?? "").trim();
    if (!periods.length) {
      return {
        unavailable: true,
        periodCount: 0,
        periods,
        message: "現在、確認できる利用予定月が設定されていません。園へご確認ください。",
      };
    }
    if (normalizedPeriodId) {
      const requested = periods.find((period) => period.id === normalizedPeriodId);
      if (!requested) {
        return {
          unavailable: true,
          periodCount: periods.length,
          periods,
          message: "指定された利用予定月は確認できません。画面を再読み込みしてください。",
        };
      }
      return { unavailable: false, period: requested, periods, suggestedTargetMonth: suggestedParentTargetMonth(currentDate()) };
    }

    const suggestedTargetMonth = suggestedParentTargetMonth(currentDate());
    const suggested = periods.find((period) => period.target_month === suggestedTargetMonth);
    if (!suggested) {
      return {
        unavailable: true,
        periodCount: periods.length,
        periods,
        message: `${suggestedTargetMonth.replace("-", "年")}月の利用予定がまだ用意されていません。園へご確認ください。`,
      };
    }
    return { unavailable: false, period: suggested, periods, suggestedTargetMonth };
  }
  ...
  function familySubmissionAccess(familyId, period, at = currentDate()) {
    ...
    const hasSubmittedVersion = Boolean(submission?.latest_submitted_version_id);
    const resubmissionAllowed = hasSubmittedVersion
      && submission.resubmission_allowed_for_version_id === submission.latest_submitted_version_id;
    const submissionLocked = hasSubmittedVersion && !resubmissionAllowed;
    const allowed = familyAvailable && (periodOpen || resubmissionAllowed) && !submissionLocked;
  ...
  function familySubmission(familyId, periodId) {
    return database.prepare(
      `SELECT id, family_id, submission_period_id, status, submitted_at,
              ...
              resubmission_allowed_for_version_id,
  ...
  function closuresForMonth(targetMonth) {
    if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(targetMonth)) {
      throw new AuthError("INVALID_TARGET_MONTH", "対象月の指定が正しくありません。");
    }
    const [year, month] = targetMonth.split("-").map(Number);
    const nextMonth = shiftedMonth(year, month, 1);
    return database.prepare(
      `SELECT date, name, type, parent_input_allowed, note
       FROM closure_days
       WHERE date >= ? AND date < ?
       ORDER BY date`,
    ).all(`${targetMonth}-01`, `${nextMonth}-01`);
  }
  ...
  function createParentSubmissionVersion({ actor, submission, period, children, closuresByDate, timestamp }) {
    ...
    for (const child of children) {
      ...
      for (const day of dailyRows(monthly.id)) {
        const isClosure = isHardClosure(day.date, closuresByDate.get(day.date));
        database.prepare(
          ...
          versionChildId,
          day.date,
          isClosure ? "closed" : day.usage_status,
          isClosure ? null : day.arrival_time,
          isClosure ? null : day.departure_time,
          isClosure ? "base" : day.source,
          isClosure ? 0 : day.changed,
          timestamp,
        );
      }
    }
  ...
  function latestSubmittedVersion(actor, input = {}) {
    assertFamilyActor(actor);
    const periodResult = targetPeriodOrUnavailable(input?.submissionPeriodId);
  ...
  function replaceDraftSchedulesFromVersion({ familyId, period, version, timestamp }) {
    const currentChildren = familyChildren(familyId, period.target_month);
    const versionChildren = new Map(version.children.map((child) => [child.childId, child]));
    let copiedChildCount = 0;
    let copiedDayCount = 0;

    for (const child of currentChildren) {
      const versionChild = versionChildren.get(child.id);
      if (!versionChild) continue;
      const monthly = monthlySchedule(child.id, period.id);
      if (!monthly) throw new AuthError("SCHEDULE_NOT_READY", "再提出用の予定を準備できませんでした。", 409);
      const versionDays = new Map(versionChild.days.map((day) => [day.date, day]));
      const draftDays = dailyRows(monthly.id);
      if (versionDays.size !== draftDays.length) {
        throw new AuthError("SUBMISSION_VERSION_INCONSISTENT", "再提出用に読み込む日別予定が不足しています。", 409);
      }
      for (const draftDay of draftDays) {
        const sourceDay = versionDays.get(draftDay.date);
        if (!sourceDay) {
          throw new AuthError("SUBMISSION_VERSION_INCONSISTENT", "再提出用に読み込む日別予定が不足しています。", 409);
        }
        database.prepare(
          `UPDATE daily_schedules
           SET usage_status = ?, arrival_time = ?, departure_time = ?, source = ?, changed = ?, updated_at = ?
           WHERE id = ?`,
        ).run(
          sourceDay.usageStatus,
          sourceDay.arrivalTime,
          sourceDay.departureTime,
          sourceDay.source,
          sourceDay.changed ? 1 : 0,
          timestamp,
          draftDay.id,
        );
        copiedDayCount += 1;
      }
      database.prepare("UPDATE monthly_schedules SET status = 'draft', updated_at = ? WHERE id = ?")
        .run(timestamp, monthly.id);
      copiedChildCount += 1;
    }

    return { copiedChildCount, copiedDayCount };
  }
  ...
  function familyMemberships(childId) {
    return database.prepare(
      `SELECT fc.family_id, f.family_code, f.display_name AS family_name,
              CASE WHEN fa.id IS NULL THEN 0 ELSE 1 END AS has_account,
       ...
       LEFT JOIN family_accounts fa ON fa.family_id = f.id
       WHERE fc.child_id = ?
  ...
  function assertFamilyExists(familyId) {
    ...
    if (!family) throw new AuthError("NOT_FOUND", "保護者・家庭が見つかりません。", 404);
    return family;
  }
  ...
  function assertResultingMembershipsWithinEnrollment(childId, profile) {
    ...
    for (const membership of memberships) {
      if (!membership.active_from) {
        throw new AuthError("MEMBERSHIP_PERIOD_INCOMPLETE", "既存の保護者・家庭の登録開始日を設定してから所属を変更してください。", 409);
      }
      if (membership.active_from < profile.enrollmentDate
        ...
        throw new AuthError("MEMBERSHIP_OUTSIDE_ENROLLMENT", "保護者・家庭の登録期間は園児の在籍期間内にしてください。", 409);
      }
    }
  }
  ...
  function administratorChildManagement(actor) {
    ...
    const families = database.prepare(
      `SELECT f.id, f.family_code, f.display_name, f.status,
              fa.login_id, f.handed_over_at, f.stop_date
       FROM families f
       LEFT JOIN family_accounts fa ON fa.family_id = f.id
       ORDER BY f.display_name, f.family_code, f.id`,
    ).all().map((family) => ({
      id: family.id,
      familyCode: family.family_code,
      displayName: family.display_name,
      status: family.status,
      loginId: family.login_id ?? null,
      startDate: family.handed_over_at ?? null,
      stopDate: family.stop_date ?? null,
      hasAccount: Boolean(family.login_id),
    }));
    ...
    const children = database.prepare(
      ...
      return {
        ...
        memberships: familyMemberships(child.id).map((membership) => ({
          ...
          hasAccount: Boolean(membership.has_account),
  ...
  function createChild(actor, input) {
    ...
    const family = profile.familyId ? assertFamilyExists(profile.familyId) : null;
    ...
    transaction(database, () => {
      ...
      writeOperation({
        ...
        detail: {
          ...
          familyName: family?.display_name ?? null,
  ...
  function updateChild(actor, childId, input) {
    ...
    if (profile.familyId) assertFamilyExists(profile.familyId);
    const originalMembership = profile.originalFamilyId
      ? database.prepare("SELECT * FROM family_children WHERE family_id = ? AND child_id = ?").get(profile.originalFamilyId, childId)
      : null;
    if (profile.originalFamilyId && !originalMembership) {
      throw new AuthError("NOT_FOUND", "編集対象の保護者・家庭の登録情報が見つかりません。", 404);
    }
    if (!originalMembership && profile.familyId) {
      throw new AuthError("FAMILY_LINK_REQUIRED", "園児情報を保存した後、家庭アカウントの発行または紐付けを行ってください。", 409);
    }
    ...
    const after = {
      ...
      membership: profile.familyId
        ? { familyId: profile.familyId, activeFrom: profile.familyActiveFrom, activeTo: profile.familyActiveTo }
        : null,
    };
    ...
    transaction(database, () => {
      ...
      if (!originalMembership) {
        // Family setup is a separate, explicit operation after child registration.
      } else if (profile.familyId === profile.originalFamilyId) {
        database.prepare(
  ...
  function saveClosureDay(actor, input) {
    requirePermission(actor, "closure-day:manage");
    const dayType = String(input?.dayType ?? "closed").trim();
    if (dayType !== "closed" && dayType !== "family_cooperation") {
      throw new AuthError("INVALID_CLOSURE_TYPE", "日付の区分が正しくありません。");
    }
    const displayName = dayType === "family_cooperation" ? "家庭保育協力日" : "休園日";
    const parentInputAllowed = dayType === "family_cooperation" ? 1 : 0;
    const date = requiredDate(input?.date, displayName);
    const targetMonth = date.slice(0, 7);
    const requestedTargetMonth = String(input?.targetMonth ?? "").trim();
    if (requestedTargetMonth && requestedTargetMonth !== targetMonth) {
      throw new AuthError("INVALID_DATE", "選択した対象月の日付を指定してください。");
    }
    if (weekdayNumber(date) === 0) {
      throw new AuthError("CLOSURE_NOT_REQUIRED", "日曜日は常に休園日のため、追加設定は不要です。", 409);
    }
    const timestamp = toIso(currentDate());
    return transaction(database, () => {
      const existing = database.prepare(
        "SELECT id, name, type, parent_input_allowed FROM closure_days WHERE date = ?",
      ).get(date);
      const closureId = existing?.id ?? randomUUID();
      if (existing) {
        database.prepare(
          `UPDATE closure_days
           SET name = ?, type = ?, parent_input_allowed = ?, note = '', updated_at = ?
           WHERE id = ?`,
        ).run(displayName, dayType, parentInputAllowed, timestamp, closureId);
      } else {
        database.prepare(
          `INSERT INTO closure_days
           (id, date, name, type, parent_input_allowed, note, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, '', ?, ?)`,
        ).run(closureId, date, displayName, dayType, parentInputAllowed, timestamp, timestamp);
      }
      if (dayType === "family_cooperation") {
        database.prepare(
          `UPDATE daily_schedules
           SET usage_status = 'off', arrival_time = NULL, departure_time = NULL,
               source = 'base', changed = 0, updated_at = ?
           WHERE date = ? AND usage_status = 'closed'
             AND monthly_schedule_id IN (
               SELECT m.id
               FROM monthly_schedules m
               JOIN submission_periods p ON p.id = m.submission_period_id
               WHERE p.target_month = ?
             )`,
        ).run(timestamp, date, targetMonth);
      }
      writeOperation({
        actor,
        operation: existing ? "closure_day.updated" : "closure_day.created",
        targetType: "closure_day",
        targetId: closureId,
        targetMonth,
        detail: {
          date,
          displayName,
          dayType,
          parentInputAllowed: Boolean(parentInputAllowed),
          previousType: existing?.type ?? null,
        },
      });
      return { id: closureId, date, name: displayName, type: dayType, parentInputAllowed: Boolean(parentInputAllowed) };
    });
  }

  function removeClosureDay(actor, input) {
    requirePermission(actor, "closure-day:manage");
    const date = requiredDate(input?.date, "休園日");
    const targetMonth = date.slice(0, 7);
    const requestedTargetMonth = String(input?.targetMonth ?? "").trim();
    if (requestedTargetMonth && requestedTargetMonth !== targetMonth) {
      throw new AuthError("INVALID_DATE", "選択した対象月の日付を指定してください。");
    ...
    const existing = database.prepare(
      "SELECT id FROM closure_days WHERE date = ?",
    ).get(date);
    if (!existing) throw new AuthError("NOT_FOUND", "解除する休園日が見つかりません。", 404);
    return transaction(database, () => {
      database.prepare("DELETE FROM closure_days WHERE id = ?").run(existing.id);
      database.prepare(
        `UPDATE daily_schedules
         SET usage_status = 'off', arrival_time = NULL, departure_time = NULL,
             source = 'base', changed = 0, updated_at = ?
      ...
      ).run(toIso(currentDate()), date, targetMonth);
      writeOperation({
        actor,
        operation: "closure_day.removed",
        targetType: "closure_day",
  ...
  function allowFamilyResubmission(actor, input) {
    requirePermission(actor, "family-submission:allow-resubmission");
    const familyId = String(input?.familyId ?? "").trim();
    const submissionPeriodId = String(input?.submissionPeriodId ?? "").trim();
    if (!familyId || !submissionPeriodId) throw new AuthError("INVALID_INPUT", "家庭と提出対象期間を指定してください。");
    const timestamp = toIso(currentDate());
    ...
    return transaction(database, () => {
      ...
      if (period.status !== "open" && period.status !== "closed") {
        throw new AuthError("PERIOD_NOT_AVAILABLE", "利用予定を確認できる対象月だけ再提出を許可できます。", 409);
      }

      const submission = familySubmission(familyId, period.id);
      if (!submission?.latest_submitted_version_id || !submission.latest_effective_version_id) {
        throw new AuthError("SUBMITTED_VERSION_REQUIRED", "再提出を許可できる提出内容がありません。", 409);
      }
      if (submission.resubmission_allowed_for_version_id === submission.latest_submitted_version_id) {
        return {
          familyId,
          submissionPeriodId: period.id,
          sourceVersionId: submission.latest_effective_version_id,
          submittedVersionId: submission.latest_submitted_version_id,
          idempotent: true,
        };
      }

      const effectiveVersion = submissionVersion(submission.latest_effective_version_id, submission);
      const copied = replaceDraftSchedulesFromVersion({
        familyId,
        period,
      ...
      const updated = database.prepare(
        `UPDATE family_submissions
         SET resubmission_allowed_for_version_id = ?, last_updated_at = ?
         WHERE id = ? AND latest_submitted_version_id = ?
           AND resubmission_allowed_for_version_id IS NULL`,
      ).run(submission.latest_submitted_version_id, timestamp, submission.id, submission.latest_submitted_version_id);
      if (updated.changes !== 1) {
        throw new AuthError("SUBMISSION_VERSION_CHANGED", "最新の提出状態を読み直して、もう一度操作してください。", 409);
      }
      writeOperation({
        actor,
        operation: "family_submission.resubmission_allowed",
        ...
        detail: {
          ...
          submissionPeriodId: period.id,
          submittedVersionId: submission.latest_submitted_version_id,
          sourceVersionId: effectiveVersion.id,
          copied,
          performedAt: timestamp,
        },
      });
  ...
  function prepareAdministratorRevision(actor, input, { requireExpectedSource }) {
    ...
    const closuresByDate = new Map(closuresForMonth(period.target_month).map((closure) => [closure.date, closure]));
    ...
    for (const requested of requestedChanges) {
      ...
      if (isHardClosure(date, closuresByDate.get(date))) {
        throw new AuthError("LOCKED_DAY", "休園日は変更できません。", 409);
      }
  ...
  function createAdministratorRevision(actor, input) {
    ...
    const result = transaction(database, () => {
      ...
      const pointerUpdate = database.prepare(
        `UPDATE family_submissions
         SET latest_confirmed_version_id = ?, latest_effective_version_id = ?,
             resubmission_allowed_for_version_id = NULL, last_updated_at = ?
  ...
  function administratorScheduleVersion(version, closuresByDate = new Map()) {
    if (!version) return null;
    const displayNames = childDisplayNames(version.children.map((child) => ({ ...child, id: child.childId })));
    return {
      id: version.id,
      ...
      children: version.children.map((child) => ({
        childId: child.childId,
        childCode: child.childCode,
        name: displayNames.get(child.childId) ?? child.name,
        className: child.className,
        days: child.days.map((day) => {
          const isClosure = isHardClosure(day.date, closuresByDate.get(day.date));
          return {
            date: day.date,
            usageStatus: isClosure ? "closed" : day.usageStatus,
            arrivalTime: isClosure ? null : day.arrivalTime,
            departureTime: isClosure ? null : day.departureTime,
          };
        }),
      })),
    };
  ...
  function administratorScheduleDashboard(actor, input = {}) {
    ...
    const requestedTargetMonth = String(input?.targetMonth ?? "").trim();
    if (requestedTargetMonth && !/^\d{4}-(0[1-9]|1[0-2])$/.test(requestedTargetMonth)) {
      throw new AuthError("INVALID_TARGET_MONTH", "対象月の指定が正しくありません。");
    }
    const todayInTokyo = tokyoCalendarParts(currentDate());
    const defaultTargetMonth = shiftedMonth(todayInTokyo.year, todayInTokyo.month, 0);
    const selectedPeriod = requestedPeriodId
      ? periods.find((period) => period.id === requestedPeriodId)
      : requestedTargetMonth
        ? periods.find((period) => period.targetMonth === requestedTargetMonth) ?? null
      : periods.find((period) => period.targetMonth === defaultTargetMonth) ?? null;
    ...
    const selectedFamily = selectedPeriod
      ? requestedFamilyId
        ? families.find((family) => family.id === requestedFamilyId)
        : families[0] ?? null
      : null;
    if (selectedPeriod && requestedFamilyId && !selectedFamily) {
    ...
    const familySummaries = selectedPeriod ? families.map((family) => {
      ...
      return {
        ...
        resubmissionAllowed: Boolean(latestSubmitted)
          && submission?.resubmission_allowed_for_version_id === latestSubmitted.id,
        extension: extension ? {
          id: extension.id,
    ...
    const selectedSubmission = selectedSummary && selectedPeriod
      ...
      : null;
    const selectedTargetMonth = selectedPeriod?.targetMonth ?? (requestedTargetMonth || defaultTargetMonth);
    const selectedClosures = closuresForMonth(selectedTargetMonth);
    const selectedClosuresByDate = new Map(selectedClosures.map((closure) => [closure.date, closure]));
    const monthlyUsageSummaries = selectedPeriod
      ? administratorScheduleExportData(actor, { submissionPeriodId: selectedPeriod.id }).children.map((child) => {
          let usageDays = 0;
          let totalMinutes = 0;
          if (child.submissionStatus === "submitted") {
            for (const day of child.days) {
              if (day.usageStatus !== "using") continue;
              const arrival = timeToMinutes(day.arrivalTime);
              const departure = timeToMinutes(day.departureTime);
              if (arrival === null || departure === null || departure <= arrival) continue;
              usageDays += 1;
              totalMinutes += departure - arrival;
            }
          }
          return {
            childId: child.childId,
            ...
            name: child.name,
            submissionStatus: child.submissionStatus,
            usageDays,
            totalMinutes,
          };
        })
    ...
    return {
      ...
      closures: selectedClosures.map((closure) => ({
        date: closure.date,
        name: closure.name,
        type: closure.type,
        parentInputAllowed: Boolean(closure.parent_input_allowed),
      })),
      latestSubmittedVersion: selectedSubmission?.latest_submitted_version_id
        ? administratorScheduleVersion(submissionVersion(selectedSubmission.latest_submitted_version_id, selectedSubmission), selectedClosuresByDate)
        : null,
      latestEffectiveVersion: selectedSubmission?.latest_effective_version_id
        ? administratorScheduleVersion(submissionVersion(selectedSubmission.latest_effective_version_id, selectedSubmission), selectedClosuresByDate)
        : null,
    };
  }

  function administratorScheduleExportData(actor, input = {}) {
    requirePermission(actor, "family-schedule:read");
    const submissionPeriodId = requiredText(
    ...
    if (!period) throw new AuthError("NOT_FOUND", "出力する対象月が見つかりません。", 404);

    const closures = closuresForMonth(period.target_month);
    const closuresByDate = new Map(closures.map((closure) => [closure.date, closure]));
    const dates = datesForMonth(period.target_month).map((date) => {
      const weekday = weekdayNumber(date);
      const closure = closuresByDate.get(date) ?? null;
      return {
        date,
        ...
        isSunday: weekday === 0,
        isClosure: isHardClosure(date, closure),
        closureName: weekday === 0 ? "日曜日" : closure?.name ?? null,
      };
    });

    const memberships = database.prepare(
      `SELECT c.id AS child_id, c.child_code, c.name, c.kana,
              c.last_name, c.first_name, c.last_name_kana, c.first_name_kana,
              c.class_name, c.birth_date, c.enrollment_date, c.withdrawal_date, c.status,
              fc.family_id, fc.active_from AS family_active_from,
              fc.active_to AS family_active_to, fc.sort_order
       FROM children c
       JOIN family_children fc ON fc.child_id = c.id
       JOIN families f ON f.id = fc.family_id
       ORDER BY c.class_name, c.kana, c.name, fc.active_from, fc.family_id`,
    ).all().filter((row) => childOverlapsMonth(row, period.target_month));

    const membershipsByChild = new Map();
    for (const membership of memberships) {
      const entries = membershipsByChild.get(membership.child_id) ?? [];
      entries.push(membership);
      membershipsByChild.set(membership.child_id, entries);
    }

    const effectiveVersionByFamily = new Map();
    function effectiveVersionForFamily(familyId) {
      if (effectiveVersionByFamily.has(familyId)) return effectiveVersionByFamily.get(familyId);
      const submission = familySubmission(familyId, period.id);
      const version = submission?.latest_effective_version_id
        ? submissionVersion(submission.latest_effective_version_id, submission)
        : null;
      effectiveVersionByFamily.set(familyId, version);
      return version;
    }

    const displayNames = childDisplayNames([...membershipsByChild.values()].map((entries) => entries[0]));
    const children = [...membershipsByChild.values()].map((childMemberships) => {
      const child = childMemberships[0];
      let hasUnsubmittedActiveDate = false;
      let hasSubmittedActiveDate = false;
      const childDays = dates.map((dateEntry) => {
        const activeMemberships = childMemberships.filter((membership) => childActiveOnDate(membership, dateEntry.date));
        if (activeMemberships.length > 1) {
          throw new AuthError(
            "CHILD_MEMBERSHIP_INCONSISTENT",
            "同じ日に複数の家庭へ所属している園児がいるため、Excelを作成できません。",
            409,
          );
        }
        if (activeMemberships.length === 0) {
          return { date: dateEntry.date, usageStatus: "not_enrolled", arrivalTime: null, departureTime: null };
        }
        if (dateEntry.isClosure) {
          return { date: dateEntry.date, usageStatus: "closed", arrivalTime: null, departureTime: null };
        }

        const membership = activeMemberships[0];
        const effectiveVersion = effectiveVersionForFamily(membership.family_id);
        const versionChild = effectiveVersion?.children.find((entry) => entry.childId === child.child_id) ?? null;
        if (!versionChild) {
          hasUnsubmittedActiveDate = true;
          return { date: dateEntry.date, usageStatus: "unsubmitted", arrivalTime: null, departureTime: null };
        }
        const versionDay = versionChild.days.find((day) => day.date === dateEntry.date);
        if (!versionDay) {
          throw new AuthError(
            "SUBMISSION_VERSION_INCONSISTENT",
            "採用中の提出版に日別予定が不足しているため、Excelを作成できません。",
            409,
          );
        }
        hasSubmittedActiveDate = true;
        return {
          date: versionDay.date,
          usageStatus: versionDay.usageStatus,
          arrivalTime: versionDay.arrivalTime,
          departureTime: versionDay.departureTime,
        };
      });

      return {
        childId: child.child_id,
        familyId: new Set(childMemberships.map((membership) => membership.family_id)).size === 1
          ? childMemberships[0]?.family_id ?? null
          : null,
        name: displayNames.get(child.child_id),
        birthDate: child.birth_date,
        className: child.class_name ?? "",
        submissionStatus: hasSubmittedActiveDate && !hasUnsubmittedActiveDate ? "submitted" : "unsubmitted",
        days: childDays,
      };
    });

    return {
      period: { id: period.id, targetMonth: period.target_month, status: period.status },
      dates,
      children,
    };
  }

  function administratorMonthlyHeadcount(actor, input = {}) {
    const data = administratorScheduleExportData(actor, input);
    const profileProblems = [];
    const children = data.children.map((child) => {
      const age = fiscalAgeOnAprilFirst(child.birthDate, data.period.targetMonth);
      if (age === null) profileProblems.push(`${child.name}: 生年月日未設定`);
      else if (age < 0 || age > 2) profileProblems.push(`${child.name}: 年度当初年齢が対象外`);
      return { ...child, ageGroup: age === null ? null : `${age}歳児` };
    });
    if (profileProblems.length) {
      throw new AuthError(
        "CHILD_PROFILE_INCOMPLETE",
        `正式な人数集計を作成できません。園児情報を確認してください: ${profileProblems.join("、")}`,
        409,
      );
    }

    const allSlots = [];
    for (let slot = OPENING_MINUTES; slot <= CLOSING_MINUTES; slot += 5) allSlots.push(slot);
    const changeTimeSet = new Set();
    const countsByDateAndSlot = [];
    const days = data.dates.map((date, dateIndex) => {
      let previousCount = { total: 0, byAgeGroup: { "0歳児": 0, "1歳児": 0, "2歳児": 0 }, childNames: [] };
      let previousStaffing = staffingForCount(previousCount);
      const changes = [];
      const slotCounts = new Map();
      let maximum = 0;
      let maximumRequiredChildcareWorkers = 0;
      let maximumRequiredLicensedNurseryTeachers = 0;
      for (const slot of allSlots) {
        const current = date.isClosure ? { total: 0, byAgeGroup: { "0歳児": 0, "1歳児": 0, "2歳児": 0 }, childNames: [] } : countAtSlot(children, dateIndex, slot);
        const staffing = staffingForCount(current);
        slotCounts.set(slot, { count: current, staffing });
        maximum = Math.max(maximum, current.total);
        maximumRequiredChildcareWorkers = Math.max(maximumRequiredChildcareWorkers, staffing.requiredChildcareWorkers);
        maximumRequiredLicensedNurseryTeachers = Math.max(
          maximumRequiredLicensedNurseryTeachers,
          staffing.requiredLicensedNurseryTeachers,
        );
        if (staffingDisplayChanged(previousStaffing, staffing)) {
          changeTimeSet.add(slot);
          changes.push({
            date: date.date,
            time: minutesToLabel(slot),
            before: previousCount.total,
            after: current.total,
            delta: current.total - previousCount.total,
            byAgeGroup: current.byAgeGroup,
            childNames: current.childNames,
            ...staffing,
          });
        }
        previousCount = current;
        previousStaffing = staffing;
      }
      countsByDateAndSlot.push(slotCounts);
      return {
        ...date,
        maximum,
        maximumRequiredChildcareWorkers,
        maximumRequiredLicensedNurseryTeachers,
        status: date.isClosure ? "closed" : changes.length ? "scheduled" : "no_schedule",
        changes,
      };
    });
    const changeSlots = [...changeTimeSet].sort((left, right) => left - right);
    const rows = changeSlots.map((slot) => ({
      time: minutesToLabel(slot),
      counts: countsByDateAndSlot.map((slotCounts) => slotCounts.get(slot).count.total),
    }));
    return {
      period: data.period,
      ageGroups: ["0歳児", "1歳児", "2歳児"],
      dates: data.dates.map(({ date, dayOfMonth, weekday, isSaturday, isSunday, isClosure, closureName }) => ({ date, dayOfMonth, weekday, isSaturday, isSunday, isClosure, closureName })),
      rows,
      days,
      staffingChangePoints: days.flatMap((day) => day.changes),
    };
  }

  function administratorStaffingRequirements(actor, input = {}) {
    const headcount = administratorMonthlyHeadcount(actor, input);
    const zeroStaffing = staffingForCount({
      total: 0,
      byAgeGroup: { "0歳児": 0, "1歳児": 0, "2歳児": 0 },
      childNames: [],
    });
    const slots = headcount.days.flatMap((day) => {
      const changesByTime = new Map(day.changes.map((change) => [change.time, change]));
      let current = zeroStaffing;
      const results = [];
      for (let slot = OPENING_MINUTES; slot <= CLOSING_MINUTES; slot += 5) {
        const time = minutesToLabel(slot);
        current = changesByTime.get(time) ?? current;
        results.push(staffingRequirementRecord(day.date, time, current));
      }
      return results;
    });
    return { period: headcount.period, slots };
  }

  function administratorQuarterHourStaffingRequirements(actor, input = {}) {
    let submissionPeriodId = String(input?.submissionPeriodId ?? "").trim();
    if (!submissionPeriodId) {
      const targetMonth = String(input?.targetMonth ?? "").trim();
      if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(targetMonth)) {
        throw new AuthError("INVALID_TARGET_MONTH", "対象月はYYYY-MM形式で指定してください。", 400);
      }
      const period = database.prepare(
        "SELECT id FROM submission_periods WHERE target_month = ?",
      ).get(targetMonth);
      if (!period) {
        throw new AuthError(
          "AUTOMATIC_SHIFT_PERIOD_NOT_FOUND",
          "この月は園児の利用予定集計に必要な提出対象期間が未登録です。",
          409,
        );
      }
      submissionPeriodId = period.id;
    }
    const requirements = administratorStaffingRequirements(actor, { submissionPeriodId });
    return {
      period: requirements.period,
      slots: aggregateStaffingRequirementsToQuarterHours(requirements.slots),
    };
  }

  function administratorQuarterHourStaffingCandidates(actor, input = {}) {
    requirePermission(actor, "staff:manage");
    const requirements = administratorQuarterHourStaffingRequirements(actor, input);
    const staff = loadStaffCandidateProfiles(database);
    return {
      period: requirements.period,
      classificationCapabilities: CURRENT_STAFF_CLASSIFICATION_CAPABILITIES,
      classificationLimitations: CURRENT_STAFF_CLASSIFICATION_LIMITATIONS,
      slots: requirements.slots.map((slot) => {
        const evaluations = staff.map((entry) => evaluateStaffAutomaticPlacementEligibilityForQuarterHourSlot(entry, slot));
        return connectRequirementWithStaffCandidates(
          slot,
          evaluations,
          CURRENT_STAFF_CLASSIFICATION_CAPABILITIES,
        );
  ...
  function dashboard(actor, input = {}) {
    assertFamilyActor(actor);
    const periodResult = targetPeriodOrUnavailable(input?.submissionPeriodId);
    if (periodResult.unavailable) {
      return {
        ...
        periods: periodResult.periods.map((period) => ({ id: period.id, targetMonth: period.target_month, status: period.status })),
      };
    }
    ...
    const children = ensured.children.map((child) => {
      ...
      const days = dailyRows(monthly.id).map((row) => {
        ...
        const closure = closuresByDate.get(row.date);
        const isClosure = isHardClosure(row.date, closure);
        return {
          ...normalized,
          usageStatus: isClosure ? "closed" : normalized.usageStatus,
          arrivalTime: isClosure ? null : normalized.arrivalTime,
          departureTime: isClosure ? null : normalized.departureTime,
          locked: isClosure || normalized.locked,
          weekday: weekdayNumber(row.date),
          closureName: isFamilyCooperationDay(closure)
            ? "家庭保育協力日"
            : isClosure ? "休園日" : null,
        };
      });
    ...
    return {
      ...
      period: {
        ...
        lockMessage: access.editable
          ? null
          : access.submissionLocked
            ? "提出済みです。変更が必要な場合は、園へLINEまたは直接ご連絡ください。"
            : !access.periodOpen
            ? "この月の受付は終了しているため、現在は閲覧のみできます。変更が必要な場合は園へご連絡ください。"
      ...
      submission: {
        ...
        resubmissionAllowed: access.resubmissionAllowed,
      ...
      children,
      history,
      periods: periodResult.periods.map((entry) => ({
        id: entry.id,
        targetMonth: entry.target_month,
        status: entry.status,
        selected: entry.id === period.id,
      })),
      suggestedTargetMonth: periodResult.suggestedTargetMonth,
    };
  }

  function updateChildSchedule(actor, childId, body) {
    assertFamilyActor(actor);
    const periodResult = targetPeriodOrUnavailable(body?.submissionPeriodId);
    if (periodResult.unavailable) throw new AuthError("TARGET_PERIOD_UNAVAILABLE", periodResult.message, 409);
    const period = periodResult.period;
    ...
    const closuresByDate = new Map(closuresForMonth(period.target_month).map((closure) => [closure.date, closure]));
    const existingRows = dailyRows(monthly.id).map((row) => {
      const isClosure = isHardClosure(row.date, closuresByDate.get(row.date));
      return isClosure ? { ...row, usage_status: "closed", arrival_time: null, departure_time: null } : row;
    });
    ...
    if (!changes.length) return dashboard(actor, { submissionPeriodId: period.id });
    ...
    return dashboard(actor, { submissionPeriodId: period.id });
  }

  function copyChildScheduleToSiblings(actor, sourceChildId, input = {}) {
    assertFamilyActor(actor);
    const periodResult = targetPeriodOrUnavailable(input?.submissionPeriodId);
    if (periodResult.unavailable) throw new AuthError("TARGET_PERIOD_UNAVAILABLE", periodResult.message, 409);
    const period = periodResult.period;
    ...
    transaction(database, () => {
      ...
      for (const child of siblingChildren) {
        ...
        for (const target of targetRows) {
          if (isHardClosure(target.date, closuresByDate.get(target.date)) || LOCKED_USAGE_STATUSES.has(target.usage_status)) continue;
  ...
  function applyBasicUsagePattern(actor, childId, input = {}) {
    assertFamilyActor(actor);
    const periodResult = targetPeriodOrUnavailable(input?.submissionPeriodId);
    if (periodResult.unavailable) throw new AuthError("TARGET_PERIOD_UNAVAILABLE", periodResult.message, 409);
    const period = periodResult.period;
  ...
  function validateFamilySchedules(familyId, period, closuresByDate) {
    const children = familyChildren(familyId, period.target_month);
    const dates = datesForMonth(period.target_month);
    for (const child of children) {
      const monthly = monthlySchedule(child.id, period.id);
      ...
      for (const row of rows) {
        if (isHardClosure(row.date, closuresByDate.get(row.date))) continue;
        if (row.usage_status === "using") validateTimePair(row.arrival_time, row.departure_time);
  ...
  function submitFamilySchedules(actor, input = {}) {
    assertFamilyActor(actor);
    const periodResult = targetPeriodOrUnavailable(input?.submissionPeriodId);
    if (periodResult.unavailable) throw new AuthError("TARGET_PERIOD_UNAVAILABLE", periodResult.message, 409);
    const period = periodResult.period;
    ...
    transaction(database, () => {
      const children = validateFamilySchedules(actor.familyId, period, closuresByDate);
      const before = familySubmission(actor.familyId, period.id);
      if (before.latest_submitted_version_id
        && before.resubmission_allowed_for_version_id !== before.latest_submitted_version_id) {
        throw new AuthError("SUBMISSION_LOCKED", "提出済みのため、園から再提出が許可されている場合だけ再提出できます。", 409);
      }
      const action = before.submitted_at ? "再提出" : "初回提出";
      const version = createParentSubmissionVersion({ actor, submission: before, period, children, closuresByDate, timestamp });
      database.prepare(
        `UPDATE family_submissions
         SET status = 'submitted', submitted_at = ?,
              latest_submitted_version_id = ?, latest_effective_version_id = ?,
              resubmission_allowed_for_version_id = NULL,
              last_updated_at = ?
         WHERE id = ?`,
  ...
  return {
    allowFamilyResubmission,
    administratorChildManagement,
    administratorRevisionHistory,
    administratorScheduleDashboard,
    administratorScheduleExportData,
    administratorMonthlyHeadcount,
    administratorQuarterHourStaffingCandidates,
    administratorQuarterHourStaffingRequirements,
    administratorStaffingRequirements,
