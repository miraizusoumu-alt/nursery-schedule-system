...
export function createFamilyScheduleService({ database, now = () => new Date() }) {
  ...
  function saveClosureDay(actor, input) {
    ...
    const submissionPeriodId = requiredText(input?.submissionPeriodId, "INVALID_PERIOD", "対象月を選択してください。");
    ...
    const period = database.prepare(
      "SELECT id, target_month FROM submission_periods WHERE id = ?",
    ).get(submissionPeriodId);
    if (!period) throw new AuthError("NOT_FOUND", "対象月が見つかりません。", 404);
    if (!date.startsWith(`${period.target_month}-`)) {
    ...
    return transaction(database, () => {
      const existing = database.prepare(
        "SELECT id, name, type, parent_input_allowed FROM closure_days WHERE submission_period_id = ? AND date = ?",
      ).get(period.id, date);
      ...
      if (existing) {
        ...
        database.prepare(
          `INSERT INTO closure_days
           ...
           VALUES (?, ?, ?, ?, ?, ?, '', ?, ?)`,
        ).run(closureId, period.id, date, displayName, dayType, parentInputAllowed, timestamp, timestamp);
      }
          ...
          `UPDATE daily_schedules
           ...
           WHERE date = ? AND usage_status = 'closed'
             AND monthly_schedule_id IN (
               SELECT id FROM monthly_schedules WHERE submission_period_id = ?
             )`,
        ).run(timestamp, date, period.id);
      }
      writeOperation({
        actor,
  ...
  function removeClosureDay(actor, input) {
    ...
    if (!period) throw new AuthError("NOT_FOUND", "対象月が見つかりません。", 404);
    const existing = database.prepare(
      "SELECT id FROM closure_days WHERE submission_period_id = ? AND date = ?",
    ).get(period.id, date);
    ...
    return transaction(database, () => {
      ...
      ).run(toIso(currentDate()), date, period.id);
      writeOperation({
        actor,
        ...
        targetMonth: period.target_month,
        detail: { submissionPeriodId: period.id, date },
      });
      return { date };
    });
  }
  ...
  function prepareAdministratorRevision(actor, input, { requireExpectedSource }) {
    ...
    const closuresByDate = new Map(closuresForPeriod(period.id).map((closure) => [closure.date, closure]));
  ...
  function administratorScheduleDashboard(actor, input = {}) {
    ...
    const selectedSubmission = selectedSummary && selectedPeriod
      ...
      : null;
    const selectedClosures = selectedPeriod ? closuresForPeriod(selectedPeriod.id) : [];
    ...
    return {
      ...
      periods,
      selectedPeriod,
      selectedTargetMonth: selectedPeriod?.targetMonth ?? (requestedTargetMonth || defaultTargetMonth),
  ...
  function administratorMonthlyHeadcount(actor, input = {}) {
    ...
    const days = data.dates.map((date, dateIndex) => {
      let previous = { total: 0, byAgeGroup: { "0歳児": 0, "1歳児": 0, "2歳児": 0 }, childNames: [] };
      const changes = [];
      let maximum = 0;
    ...
    const rows = changeSlots.map((slot) => ({
      time: minutesToLabel(slot),
      counts: data.dates.map((date, dateIndex) => date.isClosure ? 0 : countAtSlot(children, dateIndex, slot).total),
    }));
    return {
      period: data.period,
