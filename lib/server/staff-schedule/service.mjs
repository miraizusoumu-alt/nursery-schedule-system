...
export function createStaffScheduleService({ database, now = () => new Date() }) {
  function timestamp() {
    return toIso(now());
  }
  ...
  function preferenceView(staffId, date, row = null) {
    ...
    if (!row) {
      return {
        ...
        reviewMessage: null,
        weeklyAvailability,
        effectiveAvailability: weeklyAvailability?.available
          ? { source: "weekly", available: true, startTime: weeklyAvailability.startTime, endTime: weeklyAvailability.endTime }
          : { source: "weekly", available: false, startTime: null, endTime: null },
      };
    }
    let requiresAdministratorReview = false;
    let reviewMessage = null;
    if (row.preference_type === "work_time") {
      requiresAdministratorReview = !weeklyAvailability?.available
        || row.start_time < weeklyAvailability.startTime
        || row.end_time > weeklyAvailability.endTime;
      if (requiresAdministratorReview) {
        reviewMessage = "基本勤務可能時間の範囲外を含むため管理者確認が必要です。";
      }
    }
    return {
      id: row.id,
      ...
      reviewMessage,
      weeklyAvailability,
      effectiveAvailability: row.preference_type === "day_off"
        ? { source: "preference", available: false, startTime: null, endTime: null }
        : { source: "preference", available: true, startTime: row.start_time, endTime: row.end_time },
    };
  }
  ...
  function confirmMonthlySchedule(actor, input = {}) {
    ...
    const version = requireVersion(month.id, versionId);
    if (version.status === "confirmed") return scheduleDashboard(actor, { targetMonth });
    if (version.status !== "draft") throw new AuthError("INVALID_STAFF_SCHEDULE_STATUS", "シフトの状態が正しくありません。", 409);
    const occurredAt = timestamp();
    transaction(database, () => {
      database.prepare(
      ...
      writeOperation(actor, "staff_schedule_month.confirmed", month.id, targetMonth, { versionId, occurredAt });
    });
    return scheduleDashboard(actor, { targetMonth });
  }
