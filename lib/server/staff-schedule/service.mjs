...
import { availabilityCandidatesForDate, weekMaskToOrdinals } from "../staffing/availability-candidates.mjs";
...
export function createStaffScheduleService({
  ...
  function weeklyAvailabilityForDate(staffId, date) {
    ...
    ).all(row.condition_id, weekday);
    const candidates = candidateRows.length > 0
      ? candidateRows.map((candidate) => ({
          candidateId: `${row.condition_id}:${weekday}:${candidate.candidate_order}`,
      ...
      : row.available === 1
        ? [{
            candidateId: `${row.condition_id}:${weekday}:0`,
            candidateOrder: 0,
    ...
    const activeCandidates = availabilityCandidatesForDate(availability, date);
    return {
      ...availability,
      available: activeCandidates.length > 0,
      startTime: activeCandidates[0]?.startTime ?? null,
      endTime: activeCandidates[0]?.endTime ?? null,
  ...
  function preferenceView(staffId, date, row = null) {
    ...
    if (!row) {
      return {
        ...
        reviewMessage: null,
        weeklyAvailability,
        effectiveAvailability: resolveStaffEffectiveAvailability({ weeklyAvailability, preference, date }),
      };
    }
    return {
      id: row.id,
        ...
        database.prepare(
          `UPDATE staff_schedule_preferences
           SET preference_type = ?, start_time = ?, end_time = ?,
               updated_by_administrator_id = ?, updated_by_staff_account_id = NULL, updated_at = ?
           WHERE id = ?`,
