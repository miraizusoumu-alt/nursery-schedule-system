...
import { addMinutes, isIsoDate, isStartDateReached, isStopDateEffective, toIso, tokyoDateKey } from "./time.mjs";
...
export function createAuthService({ database, now = () => new Date() }) {
  ...
  function invalidateSessions(subjectType, subjectId, reason) {
    ...
    const column = subjectType === "family"
      ? "family_account_id"
      : subjectType === "staff" ? "staff_account_id" : "administrator_id";
    database.prepare(
  ...
  function staffAccountByLogin(loginId) {
    return database.prepare(
      `SELECT sa.id AS account_id, sa.staff_id, sa.login_id, sa.password_hash,
              sa.credential_version, sa.disabled_at,
              sm.name AS display_name, sm.status AS staff_status,
              sm.employment_start_date, sm.employment_end_date
  ...
  function accountCanLogin(scope, account, currentTime) {
    ...
    if (scope === "staff") {
      return account.disabled_at === null
        && account.staff_status === "active"
        && isStartDateReached(account.employment_start_date, currentTime)
        && (!account.employment_end_date || tokyoDateKey(currentTime) <= account.employment_end_date);
    }
  ...
  function insertSession(scope, account, settings) {
    ...
    database.prepare(
      `INSERT INTO auth_sessions
       (id, subject_type, family_account_id, administrator_id, staff_account_id, token_hash, csrf_token_hash,
      ...
      scope === "staff" ? account.account_id : null,
      secrets.tokenHash,
  ...
  async function login({ scope, loginId, password, source = "unknown" }) {
    if (scope !== "family" && scope !== "administrator" && scope !== "staff") {
    ...
    const account = scope === "family"
      ? familyAccountByLogin(normalizedLoginId)
      : scope === "staff" ? staffAccountByLogin(normalizedLoginId) : administratorByLogin(normalizedLoginId);
    ...
    if (scope === "family") {
      ...
      database.prepare("UPDATE staff_accounts SET last_login_at = ?, updated_at = ? WHERE id = ?")
        .run(attemptedAt, attemptedAt, account.account_id);
      writeOperation({ actor: { type: "staff", id: account.account_id }, operation: "auth.login", targetType: "staff_account", targetId: account.account_id });
    }
    return {
      actor: scope === "family"
        ...
        : scope === "staff"
          ? { type: "staff", id: account.account_id, staffId: account.staff_id, loginId: account.login_id, displayName: account.display_name, mustChangePassword: false }
          : { type: "administrator", id: account.id, role: account.role, loginId: account.login_id, displayName: account.display_name, mustChangePassword: false },
      session,
      settings,
    };
  }
  ...
  function sessionByToken(token, { touch = true } = {}) {
    ...
    const row = database.prepare(
      `SELECT s.*,
              ...
              sa.staff_id, sa.login_id AS staff_login_id,
              sa.credential_version AS staff_credential_version, sa.disabled_at AS staff_account_disabled_at,
              sm.name AS staff_display_name, sm.status AS staff_status,
              sm.employment_start_date AS staff_employment_start_date,
              sm.employment_end_date AS staff_employment_end_date
    ...
    } else if (row.subject_type === "administrator") {
    ...
    } else if (row.subject_type === "staff") {
      if (row.staff_credential_version !== row.credential_version) invalidReason = "credential_changed";
      if (row.staff_account_disabled_at || row.staff_status !== "active"
        || !isStartDateReached(row.staff_employment_start_date, currentTime)
        || (row.staff_employment_end_date && tokyoDateKey(currentTime) > row.staff_employment_end_date)) invalidReason = "account_stopped";
    } else {
      invalidReason = "invalid_subject";
    }
    if (invalidReason) {
    ...
    const actor = row.subject_type === "family"
      ...
      : row.subject_type === "staff"
        ? {
            type: "staff",
            id: row.staff_account_id,
            staffId: row.staff_id,
            loginId: row.staff_login_id,
  ...
  function logout(session) {
    ...
    const targetType = session.actor.type === "family"
      ? "family_account"
      : session.actor.type === "staff" ? "staff_account" : "administrator";
    writeOperation({ actor: session.actor, operation: "auth.logout", targetType, targetId: session.actor.id });
  }
  ...
  async function changePassword({ session, currentPassword, newPassword }) {
    ...
    const isStaff = session.actor.type === "staff";
    const table = isStaff ? "staff_accounts" : "administrators";
      ...
      const updateSql = isStaff
        ...
        : `UPDATE administrators
           SET password_hash = ?, must_change_password = 0, password_changed_at = ?,
               credential_version = credential_version + 1, updated_at = ?
           WHERE id = ?`;
      database.prepare(updateSql).run(nextHash, timestamp, timestamp, session.actor.id);
      invalidateSessions(isStaff ? "staff" : "administrator", session.actor.id, "password_changed");
      writeOperation({ actor: session.actor, operation: "account.password_changed", targetType: isStaff ? "staff_account" : "administrator", targetId: session.actor.id });
    });
    const refreshedAccount = isStaff ? staffAccountByLogin(session.actor.loginId) : administratorByLogin(session.actor.loginId);
    const refreshedSession = insertSession(isStaff ? "staff" : "administrator", refreshedAccount, settings);
  ...
  async function issueStaffAccount(actor, input) {
    requirePermission(actor, "staff:issue-account");
    const staffId = requiredText(input?.staffId, "職員", 100);
    const staff = database.prepare(
      "SELECT id, staff_code, name FROM staff_members WHERE id = ?",
    ).get(staffId);
    if (!staff) throw new AuthError("NOT_FOUND", "職員が見つかりません。", 404);
    if (database.prepare("SELECT 1 FROM staff_accounts WHERE staff_id = ?").get(staffId)) {
      throw new AuthError("ACCOUNT_EXISTS", "この職員のログインアカウントは発行済みです。", 409);
    }
    let loginId = input?.loginId ? validLoginId(input.loginId) : null;
    if (!loginId) {
      for (let attempt = 0; attempt < 10 && !loginId; attempt += 1) {
        const candidate = generateLoginId("staff");
        if (!database.prepare("SELECT 1 FROM staff_accounts WHERE login_id = ?").get(candidate)) loginId = candidate;
      }
    }
    if (!loginId) throw new AuthError("IDENTIFIER_GENERATION_FAILED", "ログイン情報を発行できませんでした。もう一度お試しください。", 503);
      ...
      transaction(database, () => {
        database.prepare(
          `INSERT INTO staff_accounts
           (id, staff_id, login_id, password_hash, temporary_password_issued_at,
            credential_version, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, 1, ?, ?)`,
        ).run(accountId, staffId, loginId, passwordHash, timestamp, timestamp, timestamp);
        writeOperation({
          actor,
          operation: "staff_account.issued",
          targetType: "staff_account",
          targetId: accountId,
          detail: { staffId, staffCode: staff.staff_code, loginId, issuedAt: timestamp },
        });
      });
    } catch (error) {
      if (String(error?.message).includes("UNIQUE")) throw new AuthError("ACCOUNT_EXISTS", "同じ職員またはログインIDのアカウントがすでに存在します。", 409);
      throw error;
    }
    return { accountId, staffId, loginId, temporaryPassword, issuedAt: timestamp };
  }

  async function reissueStaffPassword(actor, staffId) {
    requirePermission(actor, "staff:reissue-password");
    const account = database.prepare(
      "SELECT id, staff_id, login_id FROM staff_accounts WHERE staff_id = ?",
    ).get(String(staffId ?? "").trim());
    if (!account) throw new AuthError("NOT_FOUND", "職員ログインアカウントが見つかりません。", 404);
    const temporaryPassword = generateTemporaryPassword();
    const passwordHash = await hashPassword(temporaryPassword);
    const timestamp = toIso(currentDate());
    transaction(database, () => {
      database.prepare(
        `UPDATE staff_accounts
         SET password_hash = ?, temporary_password_issued_at = ?, password_changed_at = NULL,
             credential_version = credential_version + 1, updated_at = ? WHERE id = ?`,
      ).run(passwordHash, timestamp, timestamp, account.id);
      invalidateSessions("staff", account.id, "temporary_password_reissued");
      writeOperation({
        actor,
        operation: "staff_account.password_reissued",
        targetType: "staff_account",
        targetId: account.id,
        detail: { staffId: account.staff_id, loginId: account.login_id, reissuedAt: timestamp },
      });
    });
    return { accountId: account.id, staffId: account.staff_id, loginId: account.login_id, temporaryPassword, issuedAt: timestamp };
  }
